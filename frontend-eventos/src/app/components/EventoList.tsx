...
export default function EventoList({ eventos, onDelete, filtroEstado, onFiltroChange }: EventoListProps) {
  ...
  // El formato de fecha/hora se adapta al idioma activo de la aplicación
  // (es-CL -> DD de mes de AAAA, 24h · en-US -> Month DD, AAAA, AM/PM)
  const localeMap: Record<string, string> = { es: 'es-CL', en: 'en-US' };
  const locale = localeMap[i18n.language] || 'es-CL';
  ...
  const formatFechaRegistro = (isoString: string) => {
    if (!isoString) return '—';
    return new Date(isoString).toLocaleString(locale);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h2 className="text-2xl font-bold text-gray-800">{t('list.title')}</h2>
        <div className="flex items-center gap-2">
          <label className="text-sm font-medium text-gray-600">{t('list.filterLabel')}</label>
          <select
            value={filtroEstado}
          ...
          >
            <option value="Todos">{t('list.filterAll')}</option>
            {ESTADOS.map((estado) => (
              ...
              <div className="flex justify-between items-start gap-4">
                <div className="flex-1 space-y-1">
                  ...
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">{t('list.productora')}</span> {evento.nombreProductora}
                  </p>
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">{t('list.direccion')}</span> {evento.direccion}, {evento.ciudad}, {evento.pais}
                  </p>
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">{t('list.inicio')}</span> {formatFecha(evento.fechaInicio)}
                  </p>
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">{t('list.termino')}</span> {formatFecha(evento.fechaTermino)}
                  </p>
                  <p className="text-xs text-gray-400">
                    {t('list.registrado')} {formatFechaRegistro(evento.fechaRegistro)}
                  </p>
                </div>
                <button
